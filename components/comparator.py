...
class Comparator(Component):
    def __init__(self, name: str, fault_atom: Symbol, input_ports: list = None, output_port: Symbol = None):
        ...
        :param outpu_ports: lsit of symbols corresponding to the comparator's output ports
        """
        ...
        super(Comparator, self).__init__(name, ComponentType.COMPARATOR, input_ports, output_port,
                                         fault_atoms=[fault_atom])
        # Define nominal behaviour
        nom_behaviour = Implies(Equals(
                                self._input_ports[0],
                                self._input_ports[1]
                                ),
                                Equals(
...
if __name__ == "__main__":
    c1 = Comparator("C1-CMP.C", Symbol("F"))
    print(c1.behaviour_formula.serialize())

    c2 = Comparator("C1-CMP.C", Symbol("F"), [Symbol("I0", REAL), Symbol("I1", REAL)])
    print(c2.behaviour_formula.serialize())
...
class Csa(Component):
    ...
    def get_qe_formula(self):
        ...
        if not os.path.exists(formula_fn) or not os.path.exists(atoms_fn):
            ...
            atoms = {}
            atoms["f_atoms"] = self._fault_atoms
            atoms["i_concr"] = self._concretizer.input_ports
            atoms["o_abstr"] = self._abstractor.output_ports
            # Save dictionary

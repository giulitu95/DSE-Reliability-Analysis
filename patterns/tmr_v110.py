...
class TmrV110Definition(PatternDefinition):
    ...
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        ...
        super(TmrV110Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.TMR_V110)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV110Definition':
        return TmrV110Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], Symbol("EMPTY_F3"))

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV110(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class TmrV110(Pattern):
    """
    Represent a pattern of type tmr-v110
    """
    n_f_atoms = 4
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        # Output port of the pattern corresponds to the output port of the voter and the output ports of Module 3
        output_ports = []
        ...
        super(TmrV110, self).__init__(pattern_name, PatternType.TMR_V110, modules_fault_atoms + [voter_fault_atom], modules, output_ports)        

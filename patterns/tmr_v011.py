...
class TmrV011Definition(PatternDefinition):
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_f_atoms: list, voters_f_atoms: list):
        ...
        super(TmrV011Definition, self).__init__(comp_name,  comp_n_inputs, modules_f_atoms + voters_f_atoms, PatternType.TMR_V011)
...
class TmrV011(Pattern):
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_fault_atoms: list, voters_fault_atoms: list, nominal_mod_beh: Symbol):
        ...
        """
        pattern_name = comp_name + "." + PatternType.TMR_V011.name
        ...
        # Output port of the pattern corresponds to the output ports of Module 1, of the voter, and of the voter
        output_ports = modules_out_ports
        output_ports[1] = voters[0].output_ports[0]
        output_ports[2] = voters[1].output_ports[0]
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs"
        super(TmrV011, self).__init__(pattern_name, PatternType.TMR_V011, modules_fault_atoms + voters_fault_atoms,

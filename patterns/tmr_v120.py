...
class TmrV120Definition(PatternDefinition):
    """
    Definition for pattern TMR-V120
    """
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voters_f_atoms: list):
        """
        Create a definition for pattern TMRV120
        ...
        super(TmrV120Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + voters_f_atoms, PatternType.TMR_V120)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV120Definition':
        return TmrV120Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], [Symbol("EMPTY_F" + str(idx+3)) for idx in range(3)])

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV120(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voters_f_atoms, nominal_mod_beh)
...
class TmrV120(Pattern):
    """
    Represent a pattern of type tmr-v120
    """
    n_f_atoms = 5
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voters_fault_atoms: list, nominal_mod_beh: Symbol):
        ...
        # Output1: output port of voter 1, Output2: output port of voter 2,  output 3: output port of Module 3  
        output_ports = []        

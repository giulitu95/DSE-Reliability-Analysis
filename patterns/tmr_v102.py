...
class TmrV102Definition(PatternDefinition):
    ...
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voters_f_atoms: list):
        ...
        super(TmrV102Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + voters_f_atoms, PatternType.TMR_V102)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV102Definition':
        return TmrV102Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], [Symbol("EMPTY_F" + str(idx+3)) for idx in range(3)])

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV102(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voters_f_atoms, nominal_mod_beh)
...
class TmrV102(Pattern):
    ...
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voters_fault_atoms: list, nominal_mod_beh: Symbol):
        ...
        # Output1: output port of voter 1, Output2: output port of Module 2, output 3: output port of voter 2   
        output_ports = []        
        output_ports.extend(voters[0].output_ports)
        fan_out=(module[1])
        output_ports.extend(fan_out)

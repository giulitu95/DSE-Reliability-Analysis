...
class TmrV102Definition(PatternDefinition):
    """
    Definition for pattern TMR-V102
    """
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_f_atoms: list, voters_f_atoms: list):
        """
        Create a definition for pattern TMRV102
        ...
        super(TmrV102Definition, self).__init__(comp_name, comp_n_inputs, modules_f_atoms + voters_f_atoms, PatternType.TMR_V102)
        self._pt_name = pt_type.name

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV102(self._comp_name, self._comp_n_inputs, self._modules_f_atoms, self._voters_f_atoms, nominal_mod_beh)
...
class TmrV102(Pattern):
    """
    Represent a pattern of type tmr-v102
    """
    n_f_atoms = 5
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_fault_atoms: list, voters_fault_atoms: list, nominal_mod_beh: Symbol):
        ...
        """
        pattern_name = comp_name + "." + PatternType.TMR_V102.name
        ...
        output_ports = []
        output_ports.append(voters[0].output_ports[0])
        output_ports.append(modules_out_ports[1])
        output_ports.append(voters[1].output_ports[0])
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs"
        super(TmrV102, self).__init__(pattern_name, PatternType.TMR_V102, modules_fault_atoms + voters_fault_atoms, modules, output_ports)
...
if __name__ == "__main__":
    pt102_def = TmrV102Definition("C1", 1, [Symbol("F_M0"), Symbol("F_M1"), Symbol("F_M2")], [Symbol("F_V0"), Symbol("F_V1")])
    pt = pt102_def.create(Symbol("beh", FunctionType(REAL, [REAL])))

...
class TmrV111x3Definition(PatternDefinition):
    """
    Definition for pattern TMR-V111x3
    """
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        """
        Create a definition for pattern TMRV111x3
        ...
        super(TmrV111x3Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.TMR_V111x3)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV111x3Definition':
        return TmrV111x3Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], Symbol("EMPTY_F3"))

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV111x3(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class TmrV111x3(Pattern):
    """
    Represent a pattern of type tmr-v111x3
    """
    n_f_atoms = 4
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        super(TmrV111x3, self).__init__(pattern_name, PatternType.TMR_V111x3, modules_fault_atoms + [voter_fault_atom], modules, output_ports)

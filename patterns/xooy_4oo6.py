...
class XooY_4oo6_Definition(PatternDefinition):
    """
    Definition for pattern 4-o-o-6
    """
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        """
        Create a definition for pattern 4-o-o-6
        ...
        assert len(modules_f_atoms) == 5, "[" + pt_name + "] pattern has 5 modules, a correct number of fault atoms"
        self._voter_f_atom = voter_f_atom
        super(XooY_4oo6_Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.XooY_4oo6)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV123Definition':
        return XooY_4oo6_Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(6)], Symbol("EMPTY_F6"))

    def create(self, nominal_mod_beh) -> Pattern:        
        return XooY_4oo6(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class XooY_4oo6(Pattern):
    """
    Represent a pattern of type 4-o-o-6
    """
    n_f_atoms = 7
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        :param modules_fault_atoms: list of fault atoms associated to the 6 modules
        ...
        modules = [FaultyModule(pattern_name + ".M" + str(idx), comp_n_inputs, modules_fault_atoms[idx], nominal_mod_beh) for idx in range(6)]                
        
        # The output of the modules are the inputs of the voters
        modules_out_ports = []
        for module in modules:
            modules_out_ports.extend(module.output_ports)
        assert len(modules_out_ports) == 6, "[" + pattern_name + "] The voters must have 6 inputs"
        ...
        super(XooY_4oo6, self).__init__(pattern_name, PatternType.XooY_4oo6, modules_fault_atoms + [voter_fault_atom], modules, output_ports)

...
class TmrV123Definition(PatternDefinition):
    """
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_f_atoms: list, voters_f_atoms: list):
        ...
        assert len(voters_f_atoms) == 3, "[" + pt_type.name + "] pattern has 3 voters, choose a correct number of fault atoms"
        super(TmrV123Definition, self).__init__(comp_name, comp_n_inputs, modules_f_atoms + voters_f_atoms, PatternType.TMR_V123)
        self._pt_name = pt_type.name

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV123(self._comp_name, self._comp_n_inputs, self._modules_f_atoms, self._voters_f_atoms, nominal_mod_beh)
...
class TmrV123(Pattern):
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_fault_atoms: list, voters_fault_atoms: list, nominal_mod_beh: Symbol):
        ...
        """
        pattern_name = comp_name + "." + PatternType.TMR_V123.name
        ...
        voters = [Voter(pattern_name + ".V" + str(idx), voters_fault_atoms[idx], input_ports=modules_out_ports) for idx in range(3)]

        # Output of the pattern corresponds to the output ports of the voters
        output_ports = []
        for voter in voters:
            output_ports.extend(voter.output_ports)
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs"
        super(TmrV123, self).__init__(pattern_name, PatternType.TMR_V123, modules_fault_atoms + voters_fault_atoms, modules, output_ports)

...
class TmrV100Definition(PatternDefinition):
    """
    Definition for pattern TMR-V100
    """
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        """
        Create a definition for pattern TMRV100
        ...
        super(TmrV100Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.TMR_V100)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV100Definition':
        return TmrV100Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], Symbol("EMPTY_F3"))

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV100(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class TmrV100(Pattern):
    """
    Represent a pattern of type tmr-v100
    """
    n_f_atoms = 4
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        # Output port of the pattern corresponds to the output port of the voter and the output ports of Module 2 and Module 3
        output_ports = []
        output_ports.extend(self._voter.output_ports) #1
        fan_outs=(module[1], module[2]) 
        output_ports.extend(fan_outs) #0 0
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs" 
        super(TmrV100, self).__init__(pattern_name, PatternType.TMR_V100, modules_fault_atoms + [voter_fault_atom], modules, output_ports)

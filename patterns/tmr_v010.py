...
class TmrV010Definition(PatternDefinition):
    ...
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        ...
        super(TmrV010Definition, self).__init__(comp_name, pt_name, comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.TMR_V010)
        self._pt_name = pt_name

    def get_dummy_definition(self) -> 'TmrV010Definition':
        return TmrV010Definition("EMPTY", self.comp_n_inputs, [Symbol("EMPTY_F" + str(idx)) for idx in range(3)], Symbol("EMPTY_F3"))

    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV010(self._comp_name, self._pt_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class TmrV010(Pattern):
    ...
    def __init__(self, comp_name: str, pt_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        output_ports = []        
        fan_out1=(module[0])
        output_ports.extend(fan_out1)   #0
        output_ports.extend(self._voter.output_ports) #1
        fan_out2=(module[2])
        output_ports.extend(fan_out2)   #0
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs"
        super(TmrV010, self).__init__(pattern_name, PatternType.TMR_V010, modules_fault_atoms + [voter_fault_atom], modules, output_ports)

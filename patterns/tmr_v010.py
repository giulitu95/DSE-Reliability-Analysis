...
class TmrV010Definition(PatternDefinition):
    """
    Definition for pattern TMR-V010
    """
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_f_atoms: list, voter_f_atom: Symbol):
        """
        Create a definition for pattern TMRV010
        ...
        super(TmrV010Definition, self).__init__(comp_name,  comp_n_inputs, modules_f_atoms + [voter_f_atom], PatternType.TMR_V010)
    ...
    def create(self, nominal_mod_beh) -> Pattern:
        return TmrV010(self._comp_name, self._comp_n_inputs, self._modules_f_atoms, self._voter_f_atom, nominal_mod_beh)
...
class TmrV010(Pattern):
    """
    Represent a pattern of type tmr-v010
    """
    n_f_atoms = 4
    ...
    def __init__(self, comp_name: str, comp_n_inputs: int, modules_fault_atoms: list, voter_fault_atom: Symbol, nominal_mod_beh: Symbol):
        ...
        """
        pattern_name = comp_name + "." + PatternType.TMR_V010.name
        ...
        # Output port of the pattern corresponds to the output ports of Module 1, of the voter, and of output port of Module 3
        output_ports = modules_out_ports
        output_ports[1] = self._voter.output_ports
        assert len(output_ports) == 3, "[" + pattern_name + "] The pattern must have 3 outputs"
        super(TmrV010, self).__init__(pattern_name, PatternType.TMR_V010, modules_fault_atoms + [voter_fault_atom],
...
if __name__ == "__main__":
    ...
    tmr = TmrV010("C1", 1, [Symbol("F0"), Symbol("F1"), Symbol("F2")], Symbol("F3"), nominal_beh)
    print(tmr.behaviour_formula.serialize())
'''
if __name__ == "__main__":
    pt010_def = TmrV010Definition("C1", 1, [Symbol("F_M0"), Symbol("F_M1"), Symbol("F_M2")], Symbol("F_V"))
    pt = pt010_def.create(Symbol("beh", FunctionType(REAL, [REAL])))
